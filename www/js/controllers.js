...
.factory('apiService', function($http) {
    ...
    return {
        ...
        pedir: function(nombre, telefono, email, direccion, deviceToken) {
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.open("POST", "http://www.porthospub.com/simpanel/ajax/json.php?funcion=pedir", false);
            xmlhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xmlhttp.send("nombre=" + nombre + "&telefono=" + telefono + "&email=" + email + "&direccion=" + direccion + "&items=" + localStorage.getItem('cart') + "&deviceToken" + deviceToken);
            return JSON.parse(xmlhttp.responseText);
        }
    }
})
...
.controller('PlatosCtrl', function($scope, $stateParams, apiService, $state, $ionicLoading) {
    ...
    .controller('adicionesCtrl', function($scope, $stateParams, apiService, $ionicHistory, $ionicPopup) {
        ...
        if (plato.rows[0].NUMERODECARNES == 'S') {
            ...
            var myPopup = $ionicPopup.show({
                template: '<div ng-show="showinput"><h4 style="color:white;">Cuantas?</h4><input placeholder="Cuantas?" type="tel" ng-model="data.wifi"></div>',
                ...
                for (n = 0; n < adicion.length; n++) {
                    ...
                    for (a = 0; a < rows.length; a++) {
                        if (id == rows[a].IDADICION) {
                            ...
                            /*$scope.adiciones = 'Adición de ' + $scope.adiciones + rows[a].NOMBRE + ' valor: ' + rows[a].PRECIO + '<a ng-click="borrarAdicion('+id+', '+i+');" style="color:white;" class="tab-item" href="#"><i class="icon ion-ios-minus-outline"></i></a><br/>';*/
                            if (priceadicion) {
...
.controller('checkoutCtrl', function($scope, $stateParams, apiService, $state, $ionicPopup, $ionicPopover) {
    ...
    $scope.pedir = function() {
        var results = apiService.pedir($scope.formData.nombre, $scope.formData.telefono, $scope.formData.telefonofijo, $scope.formData.email, $scope.formData.direccion, $scope.deviceToken);
        console.log(results);
