                ...
                for (n = 0; n < adicion.length; n++) {
                    ...
                    for (a = 0; a < rows.length; a++) {
                        if (id == rows[a].IDADICION) {
                            ...
                            if ($scope.adiciones == '')
                                $scope.adiciones = 'Adición de ' + $scope.adiciones + rows[a].NOMBRE;
                            else
                                $scope.adiciones = $scope.adiciones + ', ' + rows[a].NOMBRE;
...
.controller('checkoutCtrl', function($scope, $stateParams, apiService, $state, $ionicPopup, $ionicPopover) {
    ...
    $scope.pedir = function() {
        var results = apiService.pedir($scope.formData.nombre, $scope.formData.telefono, $scope.formData.email, $scope.formData.direccion, $scope.formData.deviceToken);
        console.log(results);
        localStorage.setItem('cart', null);
        // An alert dialog
